...
import {
  ArrowLeft, Phone, IndianRupee, GraduationCap, FileText,
  Edit, Clock, Award, TrendingUp, MoreVertical, ChevronRight, BarChart3, Trash2, Bell, Move,
  Download, FileCheck, Printer,
  Camera, AlertCircle,
} from "lucide-react";
import {
...
import { getNextClass } from "./components/StudentDashboardHelpers";
...
import StudentDashboardModals from "./components/StudentDashboardModals";

// ============================================================================
// STUDENT DASHBOARD - MAIN ORCHESTRATOR
// Manages state and data; delegates rendering to tab/sidebar/modal components
// ============================================================================
...
export default function StudentDashboard() {
  ...
  const attendanceStartDate = useMemo(() => {
    ...
    return startYear ? `${startYear}-04-01` : `${new Date().getFullYear()}-01-01`;
  }, [currentAcademicYear]);
  const attendanceEndDate = useMemo(() => {
    ...
    return endYear ? `${endYear}-03-31` : `${new Date().getFullYear()}-12-31`;
  }, [currentAcademicYear]);
  ...
  const handlePhotoSave = async (croppedBlob) => {
    ...
    } catch (error) { console.error('Failed to update photo:', error); toast.error(t('toast.error.failedToUpdatePhoto')); }
  ...
  const handleRemovePhoto = async () => {
    ...
    } catch (error) { console.error('Failed to remove photo:', error); toast.error(t('toast.error.failedToRemovePhoto')); }
  };
  ...
  const handleRecordPayment = async () => {
    ...
    if (paymentAmount > totalBalance) {
      toast.error(`Amount cannot exceed outstanding balance of ${fmt(totalBalance)}`);
      return;
    }
    ...
    try {
      ...
      await Promise.all([refetchFeeStructure(), feeHistoryQuery.refetch(), refetchStudent()]);
  ...
  const handlePromoteStudent = async () => {
    ...
    if (!nextClass) { toast.error(t('toast.error.unableToCalculateNextClass')); return; }
    try {
  ...
  const handleSendReminder = () => {
    const defaultMessage = studentFeeStructure?.totalBalance > 0
      ? `Dear ${student.parentName || 'Parent'}, fee payment of ${fmt(studentFeeStructure?.totalBalance)} is pending for ${student.name}. Please pay at your earliest convenience.`
  ...
  const handleSendReminderMessage = async () => {
    if (!reminderMessage.trim()) { toast.error(t('toast.error.pleaseEnterAMessage')); return; }
    try {
      ...
      await studentsApi.sendReminder(id, { type: 'fee', message: reminderMessage, parentPhone: resolvedPhone, parentEmail: resolvedEmail, studentName: student.name });
  ...
  // ── Send Attendance Report Handler ───────────────────────────────────────────
  const handleSendAttendanceReport = async (channel) => {
    ...
    try {
      ...
      const resolvedEmail = student.parentEmail || student.parents?.find(par => par.isParent !== false)?.email || student.parents?.[0]?.email || '';
      const resolvedPhone = student.parentPhone || student.parents?.find(par => par.isParent !== false)?.phone || student.parents?.[0]?.phone || '';
      await studentsApi.sendReminder(id, { type: 'attendance', message, channel, parentPhone: resolvedPhone, parentEmail: resolvedEmail, studentName: student.name });
...
<div class="footer">Generated on ${new Date().toLocaleString(getDateLocale())} — Confidential</div>
  ...
  const stats = [
    { label: "Academic Average", value: avgPercentage ? `${avgPercentage}%` : "N/A", subtext: results?.length ? `${results.length} exams` : "No data", icon: Award, tab: "academics", actionLabel: "View Results", loading: resultsLoading },
    { label: "Attendance", value: `${attendancePct}%`, subtext: `${attendanceStats.present} present`, icon: Clock, tab: "attendance", actionLabel: "View Details", loading: attendanceLoading },
    { label: "Fee Balance", value: fmt(feeBalance), subtext: feeBalance > 0 ? "Outstanding" : "All clear", icon: IndianRupee, tab: "fees", actionLabel: feeBalance > 0 ? "Pay Now" : "View History", loading: loadingFeeStructure },
    { label: "Class & Roll", value: student.class || "N/A", subtext: `Roll ${student.rollNo || "N/A"}`, icon: GraduationCap, navigateTo: student?.classId ? `/classes/${student.classId?._id || student.classId}` : null, actionLabel: "View Class" },
  ];
    ...
    <>
      ...
      <div className="w-full flex-1 bg-gray-50 dark:bg-zinc-950 p-6 min-h-screen">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-lg border border-gray-100 dark:border-zinc-700 p-5">
            <div className="flex flex-col sm:flex-row items-start sm:items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <Dropdown>
                  <DropdownTrigger>
                    <Button isIconOnly variant="light" aria-label="More actions" className="text-gray-400 dark:text-zinc-500"><MoreVertical size={20} /></Button>
                  </DropdownTrigger>
                  <DropdownMenu className="min-w-[180px]">
                    ...
                    <DropdownItem key="delete" className="text-red-600" startContent={<Trash2 size={14} />} onPress={() => setIsDeleteConfirmOpen(true)}>
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              {activeTab === "attendance" && (
                <StudentAttendanceTab
                  ...
                  onSendAttendanceReport={handleSendAttendanceReport}
                />
              )}
        ...
        <StudentDashboardModals
          ...
          isTcOpen={isTcOpen}
          onTcClose={onTcClose}
          isBonafideOpen={isBonafideOpen}
          onBonafideClose={onBonafideClose}
          isCharacterOpen={isCharacterOpen}
          onCharacterClose={onCharacterClose}
          isMoveClassOpen={isMoveClassOpen}
          setIsMoveClassOpen={setIsMoveClassOpen}
