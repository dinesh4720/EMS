...
import { X, Send, Loader2, Copy, Mic, MicOff, ChevronDown } from 'lucide-react';
...
export default function AiModal({ isOpen, onClose }) {
    const [messages, setMessages] = useState([
        { role: 'assistant', content: "Hi, I'm your school AI assistant. How can I help with school work today?" }
    ]);
    ...
    const handleSend = async () => {
        ...
        try {
            const conversationHistory = messages.filter((message) => message.content);
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="relative mt-1">
                                        <button
                                            ...
                                            className="flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors disabled:opacity-60"
                                        >
                                    ...
                                    {prebuiltPrompts.map((prompt) => (
                                        ...
                                        >
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="text-sm font-semibold">{prompt.icon}</span>
                        ...
                        <div className="p-4 bg-white/80 dark:bg-black/80 border-t border-gray-100 dark:border-white/10 backdrop-blur-md">
                            <div className="relative flex items-end gap-2 bg-gray-50 dark:bg-zinc-900 rounded-3xl border border-gray-200 dark:border-zinc-700 focus-within:border-orange-500/50 focus-within:ring-2 focus-within:ring-orange-500/10 p-2 transition-all shadow-sm">
                                <textarea
                                    ...
                                    placeholder={isTranscribing ? 'Transcribing...' : isRecording ? 'Recording...' : 'Ask about students, staff, classes, or school work...'}
                                ...
                                <button
                                    onClick={handleSend}
                                    disabled={!input.trim() || isLoading || isRecording || isTranscribing || !selectedModel || !hasAvailableModels}
                            ...
                            <div className="text-center mt-2">
                                <p className="text-[10px] text-gray-400">
                                    {isRecording ? 'Recording...' : isTranscribing ? 'Transcribing...' : (selectedModelMeta ? `Powered by ${selectedModelMeta.name}` : 'School AI assistant')}
                                </p>
                            </div>
                        </div>
                    </motion.div>
