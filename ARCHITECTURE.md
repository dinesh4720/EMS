...
- ⏳ Front Desk lists have hardcoded colors and spacing (audit in progress)
...
See issue DK-204 for full details.
...
## Style Guide
Interactive style guide at `/style-guide` (dev-only route).

## IA & Checklist
Module tracker at `/ia` (dev-only route).
